...
export function ProfileView({
  ...
  useEffect(() => {
    ...
    getPlayerProfile(playerId, positionFamily)
      ...
      .catch((e) => {
        ...
        setError(
          msg === "Failed to fetch"
            ? t.common.backendConnectError
            : msg,
        );
      })
      .finally(() => setLoading(false));
  }, [playerId, positionFamily, t.common.backendConnectError, t.common.error]);
  ...
  const p = data.player;
  const xpvPerGame = Number(data.xp?.xpv_per_pass_p90 ?? p.xpv_per_pass_p90);
    ...
    <>
        ...
        <div className="pa-col pa-col-identity">
          <div className="player-card identity-card">
            ...
            {data.origin_heatmap_b64 && (
              <img src={`data:image/png;base64,${data.origin_heatmap_b64}`} alt={t.profile.passOriginAlt} className="heatmap-img" />
            )}
          </div>
        </div>
        ...
        <div className="pa-col pa-col-score">
          <div className="score-stack">
            ...
            <div className="player-card xp-profile-card">
              ...
              <XpProfileBars
                bars={data.xp_bars}
                xpvPerGame={Number.isFinite(xpvPerGame) ? xpvPerGame : null}
              />
              <XpIndicesPanel
      ...
      <div className="profile-actions">
        <Link href={`/compare?a=${playerId}`} className="btn btn-primary">
          <i className="fa-solid fa-scale-balanced" /> {t.profile.compare}
        </Link>
        <Link href={`/maps?player=${playerId}`} className="btn btn-ghost">
          <i className="fa-solid fa-map-location-dot" /> {t.profile.viewMaps}
        </Link>
      </div>
    </>
  );
